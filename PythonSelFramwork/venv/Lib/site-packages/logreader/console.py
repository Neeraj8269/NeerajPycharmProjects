...
def create_start_attr(name):
    """Creates the start attribute function"""

    def inner():
        """Begin attribute"""
        print("\033[%s" % _attrs[name][0], end='')

    globals()[name + "_start"] = inner


def create_end_attr(name):
    """Creates the end attribute function"""

    def inner():
        """Ends attribute"""
        print("\033[%s" % _attrs[name][1], end='')

    globals()[name + "_end"] = inner
...
def create_start_color(name):
    """creates start color function"""

    def inner():
        print("\033[%s" % _colors[name][0], end='')

    globals()[name + "_start"] = inner


def create_end_color(name):
    """creates end color function"""

    def inner():
        print("\033[%s" % _colors[name][1], end='')
...
_colors = {
    ...
    'light_red': ('91m', '39m'),
    'light_green': ('92m', '39m'),
    'light_yellow': ('93m', '39m'),
    'light_blue': ('94m', '39m'),
    'light_magenta': ('95m', '39m'),
    'light_cyan': ('96m', '39m'),
    'white': ('97m', '39m')
}
...
for _name in _attrs:
    create_attr(_name)
    create_start_attr(_name)
    create_end_attr(_name)
for _name in _colors:
    create_color(_name)
    create_start_color(_name)
    create_end_color(_name)
